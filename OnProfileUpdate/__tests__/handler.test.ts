...
describe("handler function", () => {
  it("should call get, insert and delete methods with no errors", async () => {
    ...
    jest
      ...
      .mockImplementation(([id, fiscalCode]) =>
        pipe(
          mockProfiles.find(
          ...
          foundProfile =>
            TE.right(
              O.fromNullable((foundProfile as unknown) as RetrievedProfile)
            )
        )
      );

    await handler(mockProfiles)(mockDependencies);
  ...
  it("should call mockLogger.error when error in upsertProfileEmail occurs", async () => {
    ...
    jest
      ...
      .mockImplementationOnce(() =>
        TE.left((new Error("Find error") as unknown) as CosmosErrors)
      );
  ...
  it("should call mockLogger.error when error in deleteProfileEmail occurs", async () => {
    const mockProfiles = [
      ...
      {
        email: "Eleanore.Kuphal2@example.net",
        fiscalCode: "VSFNVG14A39Y596X",
        isEmailEnabled: true,
        isEmailValidated: true,
        isInboxEnabled: false,
  ...
  it("should call mockDataTableProfileEmailsRepository.insert with the first validated email", async () => {
    ...
    expect(mockDataTableProfileEmailsRepository.insert).toHaveBeenCalledWith({
      email: mockProfiles[1].email,
      fiscalCode: mockProfiles[1].fiscalCode
    });
  });
});
